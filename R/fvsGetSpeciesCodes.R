...
function ()
{
  maxsp = .Fortran("fvsDimSizes",as.integer(0),as.integer(0),
           as.integer(0),as.integer(0),as.integer(0),as.integer(0))[[4]]
  ...
  for (i in 1:maxsp) {
    ans = .Fortran("fvsSpeciesCode",as.character(""),as.character(""),as.character(""),
          ...
          as.integer(0))
    ans = c(substr(ans[[1]],1,ans[[5]]),substr(ans[[2]],1,ans[[6]]),substr(ans[[3]],1,ans[[7]]))
