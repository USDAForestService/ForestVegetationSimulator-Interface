...
cat ("onSessionEnded, globals$saveOnExit=",globals$saveOnExit,
    ...
    if (globals$deleteLockFile) unlink (prjLockTxt)     
...
cat ("in updateVarSelection\n") 
    ...
    if(length(globals$lastRunVar)==1 && !is.na(match(globals$lastRunVar,vlst))){
