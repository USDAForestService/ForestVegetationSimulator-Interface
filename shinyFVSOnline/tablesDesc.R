...
and one row for each cycle year."
...
There is one row for each cycle year and species."

descFVS_DM_Sz_Sum=
"<b>FVS_DM_Sz_Sum</b> contains the number of infected trees and the number of trees killed
by mistletoe. There is one row for each cycle year, 2-inch tree DBH class." 
...
size, volume and so on, make up the columns. There is one row for every tree record and
...
standing as reported in the FVS_Treelist table. This table is output just in cycles 
...
carbon released by fire. There is one row for each cycle year."
...
is one row for each cycle year."
...
by fuel class (litter, duff, and down wood size classes). There is one row for each
cycle year."
...
fire conditions. There is one row for each cycle year. For the eastern FVS variants,
...
the number of trees by species before a fire and killed by a simulated fire reported by diameter class. 
Killed basal area and volume are also reported. There is a row for every species and cycle inwhich there is
a simulated fire."
...
classes as hard or soft down wood. There is one row for each cycle year. This table contains
...
the amount of canopy fuel by height class. That is, there is one row for each cycle
year and height class."

descFVS_SnagSum=
"<b>FVS_SnagSum</b> is generated by the Fire and Fuels Extension. The table contains
the number of hard and soft snags by tree size class. There is one row for each
cycle year."

descFVS_SnagDet=
"<b>FVS_SnagDet</b> is generated by the Fire and Fuels Extension. The table contains
detailed snag information; there is one row for each cycle year and combination of species 
and DBH class. The information includes the height of hard and soft snags, there volumes, 
the numbers per acre, and year the tree died."
...
established trees is also reported. There is a row for each cycle year and tree species."
...
values. There is one row for each cycle year. Econ extension keywords are required
to generate useful information in this table. "
...
is one line for each cycle and harvested tree. Econ extension keywords are required
to generate useful information in this table."
...
percent of roots infected. There is one row for each cycle year."
...
each root disease center and cycle year."
...
in total. There is a row for each cycle year, for each species, for trees inside disease centers that 
are infected, for those not infected, and those outside disease centers."
