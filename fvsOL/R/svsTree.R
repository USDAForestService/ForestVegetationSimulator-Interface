svsTree <- function(tree, treeform)
{
  ...
  if (tree$Cr1 && tree$Crd1 && HCB && CL) {
    ...
    if (tr$PlFrm == 0) {
        ...
        if (fallangle == 0) {
          ...
          if (length(branchList)) {
            if (length(tree$crowncolor) > 1) {
              c1 <- sort(sample.int(n = length(branchList), size = floor(length(branchList) * .75))) # nolint: line_length_linter.
      ...
      z <- rep(seq(0, 1, length = nbran) + rsc, each = 1)
      z[z > 1] <- 1
      # tlt in degrees.
      tlt <- tr$BaseUp + tltslp * z
      # tlt in slope proprtion
      ...
      startang <- runif(nbran) * 0.5 * pi
      angs <- angs + startang
      r <- tree$Crd1 / 2
      ll <- distfun(z) * 0.7  #just make them shorter.
      x <- tree$Xloc + ll * cos(angs)
      y <- tree$Yloc + ll * sin(angs)
      ll <- ll * tlt * .5
      ...
      if (nleaves > 0) {
        ...
        # Use of nubmer 5 below comes from experimentation
        nleaves <- max(5, floor(lm3 * CL * tree$Crd1 * 5))
...
fellTree <- function(fallangle, tree)
{
  ...
  tzfa <- matrix(c(cosfa, -sinfa, 0, sinfa, cosfa, 0, 0, 0, 1), nrow = 3, byrow = TRUE)   # nolint: line_length_linter.
  ...
  if (length(tree$leaves)) {
    for (ilv in seq_along(tree$leaves)) {
...
displayTrees <- function(drawnTrees) {
  ...
  for (col in names(alltr)) segments3d(alltr[[col]], col = col, lwd = 3, alpha = 1, add = TRUE) # nolint: line_length_linter.
  # make big trees cones ...
  # this code could be much faster by making a list of all triangles.
  for (tree in drawnTrees) {
    if (!is.null(tree$baseht)) {
  ...
  for (col in names(allbr)) {
    segments3d(allbr[[col]], col = col, lwd = 1, alpha = 1, add = TRUE)
  }
  for (col in names(alllv)) {
    points3d(alllv[[col]], col = col, cex = .5, alpha = 1, add = TRUE, pch = ".") #17) # nolint: line_length_linter.
  }
}

cone3d <- function(base = c(0, 0, 0), tip = c(0, 0, 1), rad = 1 , n = 8, draw.base = TRUE, # nolint: line_length_linter.
...
circle3D <- function(x0 = 0, y0 = 0, z0 = 0, r = 1, n = 60, col = "gray", alpha = 0.5, ...) { # nolint: line_length_linter.
...
matRotat <- function(mat, xa = 0, ya = 0, za = 0) {
  ...
  cosya <- cos(ya * x)
  cosza <- cos(za * x)
  mx <- matrix(c(1, 0, 0, 0, cosxa, -sinxa, 0, sinxa, cosxa), nrow = 3, byrow = TRUE) # nolint: line_length_linter.
  my <- matrix(c(cosya, 0, sinya, 0, 1, 0, -sinya, 0, cosya), nrow = 3, byrow = TRUE) # nolint: line_length_linter.
  mz <- matrix(c(cosza, -sinza, 0, sinza, cosza, 0, 0, 0, 1), nrow = 3, byrow = TRUE) # nolint: line_length_linter.
  rm <- mx %*% my %*% mz
  mat %*% rm
}
